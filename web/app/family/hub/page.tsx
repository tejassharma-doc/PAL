...
import { announceChatRead, noteMessageSeen } from '@/lib/chatSocket';
...
/* ── page ─────────────────────────────────────────────────────────────────── */
/** Coalesce a burst of arrivals into one read receipt.
 *
 * Leading edge plus a max wait, NOT a plain trailing debounce. A trailing-only
 * debounce is cleared by every new message, so in a room busier than one
 * message per interval it never fires at all — which is precisely the "badge
 * never clears" symptom this exists to fix, just needing a livelier
 * conversation to reproduce.
 */
const MARK_READ_WINDOW = 800;
let markReadTimer: ReturnType<typeof setTimeout> | null = null;
let markReadDeadline = 0;

function markReadSoon(roomId: string, onDone?: () => void): void {
  const now = Date.now();
  if (!markReadTimer) {
    // Leading edge: acknowledge at once, then hold the window open.
    markReadDeadline = now + MARK_READ_WINDOW;
    void markRoomRead(roomId).then(onDone);
    markReadTimer = setTimeout(() => {
      markReadTimer = null;
    }, MARK_READ_WINDOW);
    return;
  }
  if (now >= markReadDeadline) {
    clearTimeout(markReadTimer);
    markReadTimer = null;
    markReadSoon(roomId, onDone);
  }
}
...
export default function FamilyHubPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setMessages(msgs);
          // Tell the fallback how far we have already read, so that if it ever
          // engages it delivers only what is genuinely new instead of
          // replaying this history on top of itself.
          const newest = msgs[msgs.length - 1];
          if (newest?.created_at) noteMessageSeen(h.room_id, newest.created_at);
          announceChatRead();
        }
      } catch (e) {
  ...
  // Register the room as soon as we know it — NOT once the socket says it is
  // open. `desiredRooms` is the source of truth and every transport re-applies
  // it on connect, so gating on `connected` only created a window (and, if the
  // transport never opened, a permanent state) where the room was never
  // registered at all and no message could ever arrive.
  useEffect(() => {
    if (hub) chat.joinRoom(hub.room_id);
  }, [hub, chat]);
  ...
  useEffect(() => {
    ...
    return chat.onMessage((f: ChatFrame) => {
      ...
      const incoming: ChatMessage = {
        ...
        message_type: 'room',
        // Display only. The cursor below deliberately does NOT use this
        // fallback — a device clock running fast would poison it and the poll
        // would skip every message written in the interval, permanently.
        created_at: String(f.timestamp ?? new Date().toISOString()),
      };
      // De-dupe on message_id — our own optimistic row is reconciled here, and
      // so is the same message arriving twice if a transport hands over
      // mid-conversation (stream drops, poll covers the gap, stream returns).
      // Server timestamp or nothing. noteMessageSeen ignores an empty string.
      noteMessageSeen(hub.room_id, String(f.timestamp ?? ''));
      setMessages(prev => (prev.some(m => m.id === incoming.id) ? prev : [...prev, incoming]));

      // The user is looking at this room, so acknowledge it. `chat.markRead`
      // is a socket frame, and NO current transport lets a client send frames
      // (Centrifugo forbids publish; SSE and polling are one-way) — so it
      // returns false and the REST call is what actually does the work.
      // Without this the badge counted messages the user was watching arrive.
      if (incoming.sender_id !== myUserId) {
        // announceChatRead only zeroes the badge LOCALLY; the source of truth
        // is /chat/unread-count. Firing it before the receipt is written meant
        // the badge cleared and then reappeared on the next focus refresh if
        // the write never landed. Chain it onto the write instead.
        if (chat.markRead(incoming.id)) announceChatRead();
        else markReadSoon(hub.room_id, announceChatRead);
      }
    });
  }, [chat, hub, myUserId]);
  ...
  const handleSend = useCallback(async () => {
    ...
    // Under Centrifugo clients cannot publish (namespace `publish:false`), so
    // sendRoom() returns false and we always take the REST path — which is the
    // one that persists, authorises and PHI-redacts. On the native transport
    // the socket is tried first, exactly as before.
    let serverId: string | null = null;
    const viaSocket = chat.connected && chat.sendRoom(hub.room_id, text);
    if (!viaSocket) {
      try {
        const sent = await sendRoomMessageRest(hub.room_id, text);
        serverId = sent.message_id;
      } catch (e) {
    ...
    setMessages(prev => [
      ...prev,
      {
        // Adopt the SERVER id when we have it. Centrifugo has no per-subscriber
        // exclusion, so our own message comes back on the channel; keying the
        // optimistic row by the real id makes that echo de-dupe instead of
  ...
  // Connection state, made honest.
  //
  // This indicator existed before and said "offline" in light grey — easy to
  // miss, and it was the ONLY sign that realtime delivery had failed and the
  // user was about to be told "just refresh the page". It now carries a dot,
  // an amber colour when messages are not arriving live, and a tooltip naming
  // the actual transport, so this class of failure announces itself instead of
  // being discovered by the people using it.
  const connLabel =
    chat.state === 'open' ? 'live' : chat.state === 'connecting' ? 'connecting…' : 'catching up';
  const connColour =
    chat.state === 'open'
      ? 'var(--jade-deep)'
  ...
  const connTitle =
    chat.state === 'open'
      ? `Messages arrive instantly (${chat.transport})`
      : chat.state === 'connecting'
        ? 'Connecting to the live stream…'
        : `Live stream unavailable${chat.lastError ? ` — ${chat.lastError}` : ''}. ` +
          'New messages are still being fetched, just less immediately.';
        ...
        >
          <span
            ...
            style={{
              ...
              opacity: chat.state === 'connecting' ? 0.5 : 1,
            }}
          />
          {connLabel}
        </span>
