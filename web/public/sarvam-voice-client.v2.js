...
class SarvamVoiceClient extends EventTarget {
  ...
  onMessage(event) {
    ...
    switch (msg.type) {
      ...
      case 'state':
        ...
        break;
      case 'transcript':
        // Intercept STT transcript and call our conversation API
  ...
  async handleTranscript(msg) {
    // Call our REST API endpoint with the user's speech text
    if (!msg.text || !this.session) return;

    console.log('[Sarvam] User said:', msg.text);

    try {
      const apiUrl = this.apiBase + '/api/voice/conversation/' + this.session.session_id;
      const response = await fetch(apiUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_text: msg.text })
      });
      
      if (!response.ok) {
        console.error('[Sarvam] Conversation API error:', response.status);
        return;
      }
      
      const data = await response.json();
      console.log('[Sarvam] Assistant response:', data.assistant_response);
      
      // Emit the response so UI can display it
      this.emit('agent', { text: data.assistant_response });
      
      // Send response back to WebSocket for TTS to speak it
      ...
      console.error('[Sarvam] Failed to call conversation API:', error);
    }
  }
  ...
  onMicFrame({ pcm, rms }) {
    this.emit('level', rms);
    // Only send audio when agent is NOT speaking - complete turn-taking
    if (this.ws && this.ws.readyState === WebSocket.OPEN && !this.muted && !this.agentSpeaking) {
      this.ws.send(pcm);
    }
    // Barge-in completely disabled - mic blocked when agent speaking
  }
...
if (typeof window !== 'undefined') {
  window.SarvamVoiceClient = SarvamVoiceClient;
}
