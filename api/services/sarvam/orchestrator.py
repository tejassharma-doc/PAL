...
class VoiceSession:
    ...
    async def run(self) -> None:
        print(f"[VOICE] VoiceSession.run() started for {self.id}")
        try:
            print("[VOICE] Calling _handshake()")
    ...
    async def _pump_device(self) -> None:
        while True:
            ...
            elif kind == "speak" and msg.get("text"):
                # Handle TTS from REST API response
    ...
    async def _speak_opening(self) -> None:
        # Skip opening line - agent greeting handled by REST API
    ...
    async def _run_turn(self, text: str) -> None:
        # LLM handled by REST API - orchestrator only does STT/TTS
        # Just go back to listening immediately
        await self._set_state("listening")
        return
