...
STATUS: skipped at collection time.

This file was recovered from quote-stripping damage — every string literal in
it had lost its quotes, which broke collection for the whole integration
package. The quotes are restored below, but the module it targets does not
exist yet: `IdentityResolver`, `IdentityResolutionError`, `IDValidator` and
`ValidationError` are not defined anywhere in `services.fastmcp_client`, or
anywhere else in the repository.

So the tests cannot pass, and pretending otherwise by leaving them to fail is
worse than saying so. The import is guarded and the module skips with a reason.
When the resolver lands, delete the guard and these tests come alive as written.
"""
import pytest
...
try:
    from services.fastmcp_client import (
        IdentityResolver,
    ...
    pytest.skip(
        "Identity resolution is not implemented yet: "
        f"services.fastmcp_client does not export the resolver API ({exc}). "
        "These tests are the specification for it.",
        allow_module_level=True,
    )

from models import Doctor, Clinic  # noqa: E402
