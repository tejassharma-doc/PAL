    ...
    await db.execute(
        text("""
            INSERT INTO prescriptions (
                ...
                items, refillable, refills_remaining
            )
            VALUES (
                :id, :external_id, :patient_id, :doctor_id,
                :items::jsonb, :refillable, :refills
            )
        """),
        {
            "id": str(prescription_id),
...
async def upsert_lab_test_from_document(
    ...
    results_json = json.dumps(extracted_data.get("lab_tests", []))

    # Parse uploaded_at
    uploaded_at = None
    if document_data.get("uploadedAt"):
        try:
            uploaded_at = datetime.strptime(document_data["uploadedAt"], "%Y-%m-%d %H:%M:%S.%f")
        except:
            pass
    ...
    if row:
        ...
        await db.execute(
            ...
            """),
            {
                "id": str(lab_test_id),
                "patient_id": patient_id,
                "test_name": document_data.get("title") or document_data.get("kind", "Lab Report"),
                "results": results_json,
                "file_name": document_data.get("fileName"),
                "mime_type": document_data.get("mimeType"),
                "test_date": uploaded_at
            }
        )
        await db.flush()
    ...
    await db.execute(
        text("""
            INSERT INTO lab_tests (
                id, external_id, patient_id, test_name,
                results, file_name, mime_type, test_date
            )
            VALUES (
                :id, :external_id, :patient_id, :test_name,
                :results::jsonb, :file_name, :mime_type, :test_date
            )
        """),
        {
            "id": str(lab_test_id),
