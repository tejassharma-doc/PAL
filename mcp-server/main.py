...
POSTGRES_PASSWORD = os.getenv("POSTGRES_PASSWORD", "change_me_in_prod")
...
async def startup():
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        # Don't raise - let the app start anyway so healthcheck can report the issue
...
async def health_check():
    ...
    if db_pool:
        try:
            ...
            status["database"]["pool_size"] = db_pool.get_size()
        except Exception as e:
            status["status"] = "degraded"
            status["database"]["error"] = str(e)
    else:
        status["status"] = "degraded"
        status["database"]["error"] = "Pool not initialized"
...
# iNutriMon Webhook endpoint - NO AUTH REQUIRED
@app.post("/api/v1/webhook-inutrimon", response_model=WebhookResponse)
async def receive_inutrimon_webhook(
    request: Request,
    db: asyncpg.Connection = Depends(get_db)
):
    """
    Receive webhook data from iNutriMon system.
    Accepts any JSON structure for diet plans, nutrition data, recipes, etc.
    """
    # Get request body as JSON
    try:
        payload = await request.json()
    ...
    # Log to console
    print("========== iNUTRIMON WEBHOOK RECEIVED ==========")
    print(f"Timestamp: {timestamp.isoformat()}")
    print(f"Headers: {json.dumps(headers, indent=2)}")
    print(f"Payload: {json.dumps(payload, indent=2)}")
    print("================================================")
    ...
    try:
        # Event type from payload or default to 'nutrition_data'
        event_type = payload.get("event", "nutrition_data")

        # Source is always iNutriMon
        source = "inutrimon"
        ...
        webhook_id = str(row['id'])
        print(f"✅ iNutriMon webhook saved to database: {webhook_id}")

        # TODO: Add iNutriMon-specific processing here if needed
        # For now, just store the raw data

        return WebhookResponse(
            success=True,
            message=f"iNutriMon webhook received and stored successfully",
            webhook_id=webhook_id,
            timestamp=str(row['timestamp']),
            event_type=row['event_type'],
            dataReceived=len(payload) > 0
        ...
        print(f"❌ Error saving iNutriMon webhook: {error}")
        import traceback
        traceback.print_exc()
        ...
        return WebhookResponse(
            success=True,
            message="iNutriMon webhook received (storage pending)",
            webhook_id=webhook_id if webhook_id else None,
            dataReceived=len(payload) > 0
        )
