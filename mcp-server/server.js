...
app.post("/api/v1/webhook", wrap(async (req, res) => {
  ...
  try {
    ...
    if (payload.lab_reports && Array.isArray(payload.lab_reports)) {
    ...
    const source = headers["x-webhook-source"] || payload.source || "unknown";
    ...
    console.log(`✅ Webhook saved to database: ${savedEvent.id}`);

    // If its a medical report, log patient info
    if (eventType === "medical_report" && payload.patient_name) {
      console.log(`📋 Medical report for: ${payload.patient_name}`);
    }
    ...
    // Still return success to sender, but log the error
    res.status(200).json({
      success: true,
